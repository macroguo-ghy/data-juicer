# AvgStayTimeRange 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `AvgStayTimeRange`
- `operatorNameCn`: `直播平均停留时长环比`
- `groupName`: `live`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/live_metrics.py:416`
- DF handler：`metric_avg_stay_time_range`
...
```python
def metric_avg_stay_time_range(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    AvgStayTimeRange（直播平均停留时长环比）：
    - 计划ID：ad_live_total_stay_duration / ad_live_viewer_count 做环比（结果按秒展示）
    ...
    def item(ad_id, cur, prev, ratio):
        direction = "上升" if (ratio or 0) >= 0 else "下降"
        diff_pct = abs((ratio or 0) * 100.0)
        cur_sec = int(cur or 0)
        prev_sec = int(prev or 0)
        return f"计划ID:{ad_id}：{cur_sec}秒, 环比{direction}{diff_pct:.2f}%（上周期{prev_sec}秒）"

    if id_key == "ad_id":
        cur, prev, ratio = calc_for_ad_id(id_value)
        return (
            f"指标名称:平均停留时长（环比）, 指标值：{item(id_value, cur, prev, ratio)}"
        )
    if id_key == "adv_id":
        items = []
        for ad_id in get_main_ad_ids(state_data):
            cur, prev, ratio = calc_for_ad_id(ad_id)
            items.append(item(ad_id, cur, prev, ratio))
        return f"指标名称:平均停留时长（环比）, 指标值：{'；'.join(items)}"
    raise ValueError(f"Unsupported id for metricCode AvgStayTimeRange: {id_value}")
```
