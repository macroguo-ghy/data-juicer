...
- `operatorType`: `metric`
- `operatorNameEn`: `LiveDORate`
- `operatorNameCn`: `直播点击支付率同行数据`
- `groupName`: `live`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/live_metrics.py:314`
- DF handler：`metric_live_do_rate_bench`
...
```python
def metric_live_do_rate_bench(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    LiveDORate（直播点击支付率同行数据）：
    - 计划ID：ad_live_product_orders / ad_live_product_clicks 对比 bench_click_to_pay_rate
    - 账户ID：遍历主投计划 ad_id 逐个计算并拼接
    返回：消息内容字符串
    """
    world = state_data.get("world_state", {}) or {}
    bench = parse_percent_to_ratio(world.get("bench_click_to_pay_rate")) or 0.0
    ...
    if id_key == "ad_id":
        return f"指标名称:点击支付率（对比同行）, 指标值：{item(id_value, calc_rate(id_value))}"
    if id_key == "adv_id":
        items = [item(ad_id, calc_rate(ad_id)) for ad_id in get_main_ad_ids(state_data)]
        return f"指标名称:点击支付率（对比同行）, 指标值：{'；'.join(items)}"
    raise ValueError(f"Unsupported id for metricCode LiveDORate: {id_value}")
```
