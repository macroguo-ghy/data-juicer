# LowEfficiencyMaterialsCount 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `LowEfficiencyMaterialsCount`
- `operatorNameCn`: `低效素材数`
- `groupName`: `materials`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/materials_metrics.py:75`
- DF handler：`metric_low_efficiency_materials_count`
...
```python
def metric_low_efficiency_materials_count(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    LowEfficiencyMaterialsCount（低效素材数）：
    - 计划ID：ad_inefficient_materials_count
    - 账户ID：adv_inefficient_materials_count
    返回：消息内容字符串
    """

    def format_message(id_label, value):
        num = float(value) if isinstance(value, (int, float)) else 0.0
        return f"指标名称:低效素材数, 指标值：{id_label}:{id_value}：{num:.4f}"
    ...
    if id_key == "ad_id":
        ...
        for ad in state_data.get("ad_state", []) or []:
            if str(ad.get("ad_id", "")).strip() != id_value:
                continue
            count = ad.get("ad_inefficient_materials_count")
            break
        return format_message("计划ID", count)
    ...
    if id_key == "adv_id":
        ...
        for adv in state_data.get("adv_state", []) or []:
            if str(adv.get("adv_id", "")).strip() != id_value:
                continue
            if "adv_inefficient_materials_count" in adv:
                count = adv.get("adv_inefficient_materials_count")
            else:
                adv_metrics = adv.get("adv_metrics", {}) or {}
                count = adv_metrics.get("adv_inefficient_materials_count")
            break
        return format_message("账户ID", count)

    raise ValueError(
        f"Unsupported id for metricCode LowEfficiencyMaterialsCount: {id_value}"
    )
```
