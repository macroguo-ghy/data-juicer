...
- `operatorType`: `metric`
- `operatorNameEn`: `AdStatus`
- `operatorNameCn`: `计划是否暂停`
- `groupName`: `ad_flags`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/ad_flags.py:100`
- DF handler：`metric_ad_status`
...
```python
def metric_ad_status(state_data, tool_input, id_key, id_value, start_date, end_date):
    """
    AdStatus（计划是否暂停）：
    - 计划ID：取 ad_state[*].is_ad_paused
    ...
    def get_value(ad_id):
        for ad in state_data.get("ad_state", []) or []:
            if str(ad.get("ad_id", "")).strip() == ad_id:
                return _to_yes_no(ad.get("is_ad_paused"))
        return "否"
    ...
    if id_key == "ad_id":
        return f"指标名称:计划是否暂停, 指标值：{item(id_value, get_value(id_value))}"
    if id_key == "adv_id":
        items = [item(ad_id, get_value(ad_id)) for ad_id in get_main_ad_ids(state_data)]
        return f"指标名称:计划是否暂停, 指标值：{'；'.join(items)}"
    raise ValueError(f"Unsupported id for metricCode AdStatus: {id_value}")
```
