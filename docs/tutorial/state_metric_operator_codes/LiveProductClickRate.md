# LiveProductClickRate 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `LiveProductClickRate`
- `operatorNameCn`: `直播商品点击率同行数据`
- `groupName`: `live`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/live_metrics.py:280`
- DF handler：`metric_live_product_click_rate_bench`
...
```python
def metric_live_product_click_rate_bench(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    LiveProductClickRate（直播商品点击率同行数据）：
    - 计划ID：ad_live_product_clicks / ad_live_product_impressions 对比 bench_product_click_rate
    - 账户ID：遍历主投计划 ad_id 逐个计算并拼接
    返回：消息内容字符串
    """
    world = state_data.get("world_state", {}) or {}
    bench = parse_percent_to_ratio(world.get("bench_product_click_rate")) or 0.0
    ...
    if id_key == "ad_id":
        return f"指标名称:商品点击率（对比同行）, 指标值：{item(id_value, calc_rate(id_value))}"
    if id_key == "adv_id":
        items = [item(ad_id, calc_rate(ad_id)) for ad_id in get_main_ad_ids(state_data)]
        return f"指标名称:商品点击率（对比同行）, 指标值：{'；'.join(items)}"
    raise ValueError(f"Unsupported id for metricCode LiveProductClickRate: {id_value}")
```
