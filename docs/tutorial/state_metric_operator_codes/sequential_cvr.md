...
- `operatorType`: `metric`
- `operatorNameEn`: `sequential_cvr`
- `operatorNameCn`: `素材 CVR（环比）`
- `groupName`: `materials`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/materials_metrics.py:333`
- DF handler：`metric_sequential_cvr`
...
```python
def metric_sequential_cvr(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    sequential_cvr（素材 CVR 环比）：
    - 计划ID：ad_material_conversions / ad_material_clicks
    - 账户ID：adv_material_conversions / adv_material_clicks
    返回：消息内容字符串
    """
    if id_key == "ad_id":
        ad = next(
    ...
    if id_key == "adv_id":
        ...
        conv = (adv or {}).get("adv_material_conversions") or {}
    ...
    raise ValueError(f"Unsupported id for metricCode sequential_cvr: {id_value}")
```
