# LiveProductShowRate 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `LiveProductShowRate`
- `operatorNameCn`: `直播商品曝光率同行数据`
- `groupName`: `live`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/live_metrics.py:31`
- DF handler：`metric_live_product_show_rate`
- Data-Juicer 目标入口：`operatorCode.calculate(...)`

## Data-Juicer operatorCode 要求

同步到 ADC 元数据平台时，`operatorCode` 必须定义 `calculate(...)`。推荐写法：

```python
def calculate(state, id_value, start_date=None, end_date=None, helpers=None):
    id_key = helpers.get_id_key(state, id_value)
    ...
```

其中 `id_value`、`start_date`、`end_date` 需要在 `inputParameter.params` 和 YAML `parameter_mapping` 中显式配置；`id_key` 通过 `helpers.get_id_key(state, id_value)` 获取，不再作为形参注入。

下面保留的是 Dataset Factory 真实计算代码，迁移时需要把 `state_data` 改为 `state`，把公共数学函数改为 `helpers.xxx(...)`，并移除对 Dataset Factory 包路径的运行时依赖。

## DF 模块依赖上下文

```python
from utils.math import (
    calc_bench_compare,
    calc_ratio_from_series,
    calc_sequential_stats_for_fraction,
    parse_duration_seconds,
    parse_percent_to_ratio,
    resolve_date_range_from_series,
    sum_numeric_values_in_range,
)

from ..query_metric_data import get_main_ad_ids, register_metric

def _format_sequential_rate_item(ad_id, cur, ratio, prev):
    """环比格式化（率类指标），cur/prev 以百分比展示。"""
    direction = "上升" if (ratio or 0) >= 0 else "下降"
    diff_pct = abs((ratio or 0) * 100.0)
    prev_val = 0.0 if prev is None else float(prev)
    cur_val = 0.0 if cur is None else float(cur)
    return f"计划ID:{ad_id}：{cur_val * 100:.4f}% 环比{direction}{diff_pct:.2f}%（上周期{prev_val * 100:.4f}%）"

def _get_ad_item(state_data, ad_id):
    for ad in state_data.get("ad_state", []) or []:
        if str(ad.get("ad_id", "")).strip() == ad_id:
            return ad
    return None
```

## DF 原始计算代码

```python
def metric_live_product_show_rate(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    LiveProductShowRate（直播商品曝光率对比同行）：
    - 计划ID：商品曝光率 = ad_live_product_impressions / ad_live_watch_count（按日期区间求和）
      再与 world_state.bench_product_impression_rate 做对比，输出“高于/低于同行xx%”
    - 账户ID：遍历主投计划 ad_id，逐个按计划口径计算并拼接多条消息
    返回：消息内容字符串
    """
    world_state = state_data.get("world_state", {}) or {}
    bench_ratio = parse_percent_to_ratio(
        world_state.get("bench_product_impression_rate")
    )
    if bench_ratio is None:
        bench_ratio = 0.0

    def calc_for_ad_id(ad_id):
        ad_item = _get_ad_item(state_data, ad_id)
        if not isinstance(ad_item, dict):
            return 0.0
        impressions_map = ad_item.get("ad_live_product_impressions") or {}
        watch_map = ad_item.get("ad_live_watch_count") or {}
        s, e = resolve_date_range_from_series(
            [impressions_map, watch_map], start_date, end_date
        )
        impressions_sum = sum_numeric_values_in_range(impressions_map, s, e)
        watch_sum = sum_numeric_values_in_range(watch_map, s, e)
        if watch_sum <= 0:
            return 0.0
        return impressions_sum / watch_sum

    def format_item(ad_id, rate):
        rate_r = round(rate, 4)
        bench_r = round(bench_ratio, 4)
        diff_pct = 0.0 if bench_r == 0 else (rate_r - bench_r) / bench_r * 100.0
        cmp_word = "高于同行" if diff_pct >= 0 else "低于同行"
        diff_abs = abs(diff_pct)
        return f"计划ID:{ad_id}：{rate_r * 100:.4f}% {cmp_word}{diff_abs:.2f}%（同行均值{bench_r * 100:.4f}%）"

    if id_key == "ad_id":
        rate = calc_for_ad_id(id_value)
        return f"指标名称:商品曝光率（对比同行）, 指标值：{format_item(id_value, rate)}"

    if id_key == "adv_id":
        items = [
            format_item(ad_id, calc_for_ad_id(ad_id))
            for ad_id in get_main_ad_ids(state_data)
        ]
        return f"指标名称:商品曝光率（对比同行）, 指标值：{'；'.join(items)}"

    raise ValueError(f"Unsupported id for metricCode LiveProductShowRate: {id_value}")
```
