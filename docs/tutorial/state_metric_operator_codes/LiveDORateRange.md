# LiveDORateRange 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `LiveDORateRange`
- `operatorNameCn`: `直播点击支付率环比`
- `groupName`: `live`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/live_metrics.py:204`
- DF handler：`metric_live_do_rate_range`
...
```python
def metric_live_do_rate_range(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    LiveDORateRange（直播点击支付率环比）：
    - 计划ID：ad_live_product_orders / ad_live_product_clicks 做环比
    ...
    if id_key == "ad_id":
        ...
        return f"指标名称:点击支付率（环比）, 指标值：{_format_sequential_rate_item(id_value, cur or 0.0, ratio or 0.0, prev or 0.0)}"
    ...
    if id_key == "adv_id":
        ...
        return f"指标名称:点击支付率（环比）, 指标值：{'；'.join(items)}"

    raise ValueError(f"Unsupported id for metricCode LiveDORateRange: {id_value}")
```
