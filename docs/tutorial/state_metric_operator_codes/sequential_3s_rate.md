# sequential_3s_rate 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `sequential_3s_rate`
- `operatorNameCn`: `素材3s完播率环比`
- `groupName`: `materials`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/materials_metrics.py:440`
- DF handler：`metric_sequential_3s_rate`
...
```python
def metric_sequential_3s_rate(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    sequential_3s_rate（素材3s完播率环比）：
    - 计划ID：ad_3s_completion_materials_count / ad_material_impressions 做环比
    - 账户ID：adv_3s_completion_materials_count / adv_material_impressions 做环比
    返回：消息内容字符串
    """
    if id_key == "ad_id":
        ad = next(
        ...
        numer = (ad or {}).get("ad_3s_completion_materials_count") or {}
        denom = (ad or {}).get("ad_material_impressions") or {}
        cur, prev, ratio = calc_sequential_stats_for_fraction(
            numer, denom, start_date, end_date
        )
    ...
    if id_key == "adv_id":
        ...
        numer = (adv or {}).get("adv_3s_completion_materials_count") or {}
        denom = (adv or {}).get("adv_material_impressions") or {}
        cur, prev, ratio = calc_sequential_stats_for_fraction(
            numer, denom, start_date, end_date
        )
        return _format_sequential_message(
    ...
    raise ValueError(f"Unsupported id for metricCode sequential_3s_rate: {id_value}")
```
