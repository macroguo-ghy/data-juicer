# DeclineMaterialsCount 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `DeclineMaterialsCount`
- `operatorNameCn`: `衰退素材数`
- `groupName`: `materials`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/materials_metrics.py:168`
- DF handler：`metric_decline_materials_count`
...
```python
def metric_decline_materials_count(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    DeclineMaterialsCount（衰退素材数）：
    - 计划ID：ad_declining_materials_count
    - 账户ID：adv_declining_materials_count
    返回：消息内容字符串
    """
    ...
    if id_key == "ad_id":
        for ad in state_data.get("ad_state", []) or []:
            if str(ad.get("ad_id", "")).strip() == id_value:
                value = float(ad.get("ad_declining_materials_count") or 0.0)
                break
    elif id_key == "adv_id":
        for adv in state_data.get("adv_state", []) or []:
            if str(adv.get("adv_id", "")).strip() == id_value:
                value = float(adv.get("adv_declining_materials_count") or 0.0)
                break
    else:
        raise ValueError(
            f"Unsupported id for metricCode DeclineMaterialsCount: {id_value}"
        )
    return f"指标名称:衰退素材数, 指标值：{label}:{id_value}：{value:.4f}"
```
