# PlanMarGoalIsVideoPromGoods 计算代码

- `operatorType`: `metric`
- `operatorNameEn`: `PlanMarGoalIsVideoPromGoods`
- `operatorNameCn`: `是否为直播计划`
- `groupName`: `ad_flags`
- DF 来源：`/Users/bytedance/develop/dataset_factory/tool_handlers/query_metrics/ad_flags.py:158`
- DF handler：`metric_plan_mar_goal_is_video_prom_goods`
...
```python
def metric_plan_mar_goal_is_video_prom_goods(
    state_data, tool_input, id_key, id_value, start_date, end_date
):
    """
    PlanMarGoalIsVideoPromGoods（是否为直播计划）：
    - 计划ID：取 ad_state[*].is_live_ad（缺省为 否）
    ...
    def get_value(ad_id):
        for ad in state_data.get("ad_state", []) or []:
            if str(ad.get("ad_id", "")).strip() == ad_id:
                return _to_yes_no(ad.get("is_live_ad", "否"))
        return "否"
    ...
    if id_key == "ad_id":
        return f"指标名称:跑量计划类型是否包含推直播, 指标值：{item(id_value, get_value(id_value))}"
    if id_key == "adv_id":
        items = [item(ad_id, get_value(ad_id)) for ad_id in get_main_ad_ids(state_data)]
        return f"指标名称:跑量计划类型是否包含推直播, 指标值：{'；'.join(items)}"
    raise ValueError(
        f"Unsupported id for metricCode PlanMarGoalIsVideoPromGoods: {id_value}"
    )
```
