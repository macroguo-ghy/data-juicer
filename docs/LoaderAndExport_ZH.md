...
Ray 文件导出默认不启用小文件合并。compact 仍是实验能力，支持单 target 的 `target: local/hdfs` 以及 `export.targets[]` fan-out，格式仅支持 `parquet/jsonl`。如需启用，可在单 target 的 `export.compact` 或 fan-out target 顶层显式设置 `compact: {}` 使用默认配置，默认配置为 `target_bytes_per_file: 67108864`、`target_rows_per_file: 200000`、`max_buffer_bytes: 134217728`；也可以配置字典字段覆盖默认值。包含较大二进制 payload 的 target 建议保持默认关闭，或显式设置 `compact: false`：
...
export:
  ...
  filesystem: pyarrow
  compact:
    target_bytes_per_file: 134217728
...
export:
  ...
  targets:
    - target: hdfs
      type: parquet
      path: hdfs://cluster/path/short_video
      mode: overwrite
    ...
    - target: hdfs
      type: parquet
      path: hdfs://cluster/path/long_video
      mode: overwrite
...
`compact` 只能配置在单 target 的 `export.compact` 或 `export.targets[]` 顶层，不支持 `compact: true` 或 `enabled`，也不支持 `extra_args.compact`。缺省字段会从默认值补齐；`target_bytes_per_file`、`target_rows_per_file`、`max_buffer_bytes` 都必须为正整数；`max_buffer_bytes` 默认 `2 * target_bytes_per_file`，且必须大于等于 `target_bytes_per_file`。compact 不支持 `hive/magnus/lark/tos/s3` 等非本地/HDFS文件 sink。同一个 fan-out 任务里 compact target 与默认关闭或 `compact: false` target 可以混用；如果配置多个 compact target，归一化后的 compact 配置必须完全一致，否则会提前报错。
