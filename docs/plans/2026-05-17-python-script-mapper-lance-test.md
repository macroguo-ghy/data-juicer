...
tables. The test script reads nested fields from the current sample and returns a small flat output object.
...
- drop the original nested fields from the output to avoid Lance struct field-order issues
...
The output table only keeps `__adc_record_key` and scalar fields generated by the script. Dropping the original nested
fields makes this smoke test easier to run because Lance/PyArrow does not need to cast dynamic nested structs back into
the output schema.
...
The script extracts fields from the nested sample and returns a new flat dictionary. Keep `__adc_record_key` in the
output so record callbacks and result tracing can still identify the row.
  ...
  - python_script_mapper:
      ...
      python_code: |
        ...
        def process(sample, context):
            ...
            return {
                "__adc_record_key": sample.get("__adc_record_key", ""),
                "script_user_query": user_query,
                "script_query_id": matched.group(0) if matched else "",
                "script_unique_id": str(meta_info.get("unique_id") or ""),
                "script_scenario_id": scenario.get("scenario_id") or "",
                "script_business_line": scenario.get("business_line") or "",
                "script_has_response": input_obj.get("response") is not None,
                "script_user_account": context["ctx"].get("userAccount", ""),
            }
```

## 5. Expected Output

The output row should contain only these fields:
...
- The output Lance table contains only `__adc_record_key` and scalar script fields.
...
- No original nested object field is returned by the script.
...
- The mapper returns exactly the dictionary returned by `process(sample, context)`. This guide returns a new flat
  dictionary instead of the original sample to avoid nested Lance schema issues.
