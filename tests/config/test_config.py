...
class ConfigTest(DataJuicerTestCaseBase):
    ...
    def test_structured_export_quota_mode_requires_ray_executor(self):
        config_data = {
            'project_name': 'structured_export_quota_requires_ray',
            'dataset_path': './tests/core/data/test_data/sample.jsonl',
            'export': {
                'target': 'local',
                'path': './outputs/max_rows.jsonl',
        ...
        try:
            with self.assertRaisesRegex(ValueError, 'quota_reservation'):
                init_configs(args=['--config', temp_config], load_configs_only=True)
        finally:
            os.unlink(temp_config)

    def test_structured_export_quota_mode_accepts_ray_executor(self):
        config_data = {
            'project_name': 'structured_export_quota_ray',
            'executor_type': 'ray',
            'dataset_path': './tests/core/data/test_data/sample.jsonl',
            'export': {
                'target': 'local',
        ...
        try:
            cfg = init_configs(args=['--config', temp_config], load_configs_only=True)
            self.assertEqual(cfg.export.max_rows_mode, 'quota_reservation')
            self.assertEqual(cfg.export.max_rows_quota_batch_size, 4)
        finally:
            os.unlink(temp_config)

    def test_structured_export_max_rows_mode_requires_max_rows(self):
        config_data = {
            'project_name': 'structured_export_mode_without_rows',
            'dataset_path': './tests/core/data/test_data/sample.jsonl',
            'export': {
                'target': 'local',
                'path': './outputs/max_rows.jsonl',
        ...
        try:
            with self.assertRaisesRegex(ValueError, 'max_rows_mode'):
                init_configs(args=['--config', temp_config], load_configs_only=True)
        finally:
            os.unlink(temp_config)

    def test_structured_export_quota_batch_size_requires_quota_mode(self):
        config_data = {
            'project_name': 'structured_export_quota_batch_without_mode',
            'dataset_path': './tests/core/data/test_data/sample.jsonl',
            'export': {
                'target': 'local',
                'path': './outputs/max_rows.jsonl',
        ...
        try:
            with self.assertRaisesRegex(ValueError, 'max_rows_quota_batch_size'):
                init_configs(args=['--config', temp_config], load_configs_only=True)
        finally:
            os.unlink(temp_config)
