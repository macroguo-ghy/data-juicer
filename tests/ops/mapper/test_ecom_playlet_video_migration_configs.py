...
if "av" not in sys.modules:
    ...
    av_stub.logging = SimpleNamespace(PANIC=0, set_level=lambda *_args, **_kwargs: None)
    av_stub.container = SimpleNamespace(InputContainer=object)
    av_stub.VideoFrame = object
...
class EcomPlayletVideoConfigLoadTest(unittest.TestCase):
    def test_ecom_playlet_video_hdfs_parquet_configs_are_split_by_duration(self):
        config_dir = (
            Path(__file__).resolve().parents[3]
            / "demos/bytedance/ecom_playlet_a_dragon/configs"
        )
        source_path = "hdfs://haruna/home/byte_life_gen_ai/user/wangqianle/ecom_raw/playlet/20260204"
        output_base = "hdfs://haruna/ad_base/addrd_core/addrd_stats/hdfs/ccu/ecom_playlet"
        expected = {
            "ecom_playlet_video_short_hdfs_parquet.yaml": {
                "filter": "item_duration_s <= 120",
                "output_path": f"{output_base}/20260204_full_video_short_dedup",
                "op_classes": [
                ...
                "download_timeout": 30,
                "checkpoint_dir": (
                    "hdfs://haruna/ad_base/addrd_core/addrd_stats/hdfs/checkpoint/ccu/"
                    "ecom_playlet/20260204_full_video_short_dedup"
                ),
                "avoid_write_fusion": False,
                "min_rows_per_file": 1000,
                "max_rows_per_file": 1500,
            },
            "ecom_playlet_video_long_hdfs_parquet.yaml": {
                "filter": "item_duration_s > 120",
                "output_path": f"{output_base}/20260204_video_long",
                "op_classes": [
                    "GeneralFieldFilter",
                    "JsonObjectMapper",
                    "FieldAssignMapper",
        ...
        for config_name, tuning in expected.items():
            with self.subTest(config_name=config_name):
                ...
                cfg = init_configs(args=["--config", str(path)], load_configs_only=True)
                ...
                self.assertEqual(dataset_config["path"], source_path)
                ...
                self.assertTrue(dataset_config["skip_zero_row_group_files"])
                ...
                self.assertEqual(ops[2].assignments["id"]["template"], "vid-{video_id}")
                self.assertEqual(ops[2].assignments["source"]["value"], "ecom_playlet_raw_data")

                if tuning["has_download"]:
                    self.assertEqual(ops[3].vid_key, "video_id")
                    ...
                    self.assertEqual(cfg.ray_data_checkpoint["dir"], tuning["checkpoint_dir"])
