...
from data_juicer.core.task_notification import TaskNotificationManager
...
def _configured_export_schema_columns(cfg) -> Optional[list[str]]:
    export_cfg = getattr(cfg, "export", None)
    if isinstance(export_cfg, dict):
        schema_config = export_cfg.get("schema")
    else:
    ...
    schema = build_arrow_schema_from_config(schema_config)
    return list(schema.names) if schema is not None else None
...
class RayExecutor(ExecutorBase, DAGExecutionMixin, EventLoggingMixin):
    ...
    def run(self, load_data_np: Optional[PositiveInt] = None, skip_export: bool = False, skip_return: bool = False):
        ...
        """
        notification_manager = TaskNotificationManager(self.cfg)
        notification_manager.set_export_summary_provider(self._current_export_summary)
        ...
        try:
            ...
            with RayDataCheckpointManager(self.cfg) as ray_data_checkpoint:
                ...
                # 1. load data
                notification_manager.update_phase("load")
                ...
                with TempDirManager(self.tmp_dir):
                    ...
                    notification_manager.update_phase("process")
                    ...
                    if not skip_export and not dry_run_plan:
                        notification_manager.update_phase("export")
                        logger.info("Exporting dataset to disk...")
                        schema_columns = _configured_export_schema_columns(self.cfg)
                        self.exporter.export(dataset.data, columns=schema_columns)
                        export_summary = getattr(self.exporter, "last_export_summary", None)
    ...
    def _current_export_summary(self):
        current_export_summary = getattr(self.exporter, "current_export_summary", None)
        if callable(current_export_summary):
            return current_export_summary()
        return getattr(self.exporter, "last_export_summary", None)
