...
4. Submit through `online_ray_job.py launch` for one-off Federal jobs. For one-off Federal jobs, use repository-relative Data-Juicer entrypoints such as `python tools/process_data_base64.py ...` because the platform checks out the repo as the working directory. For resident / long-running Ray clusters, use `SubmitRayJobToLongRunningCluster` from `references/long_running_cluster.md` and absolute Data-Juicer paths.
5. Poll Federal status, but treat Ray driver terminal state as authoritative when Federal status lags. Launching a fresh Ray cluster commonly takes minutes; `LAUNCHING` for up to 10 minutes is normal and should not be treated as a blocker by itself.
6. Open Ray UI / Ray History and inspect Jobs first. If the History Server URL gains a suffix like `<cluster>:<log_suffix>`, use that full key for all History API calls. Do not conclude that no driver started from an empty `/history/<cluster>/api/jobs` response without checking the suffixed key shown by the Ray UI. Read driver stdout/stderr before worker logs.
